    ...
    override func spec() {
        ...
        describe("managerByScene") {
            ...
            context("Scene1、Scene2両方を同じManagerに登録している場合") {
                beforeEach {
                    manager.set(scene: mockScene1, stuff: obj1)
                    manager.set(scene: mockScene2, stuff: obj2)
                }
                it("Scene1は取得できる") {
                    let result = SceneManager.managerByScene(scene: mockScene1)
                    expect(result).toNot(beNil())
            ...
            context("Scene1, Scene2それぞれ違うManagerに登録されている場合") {
                ...
                beforeEach {
                    manager.set(scene: mockScene1, stuff: obj1)
                    manager2.set(scene: mockScene2, stuff: obj2)
                }
        ...
        describe("dispose") {
            ...
            context("SceneManagerにすでにSceneが登録されていた場合") {
                ...
                it("disposeした後は取得できない") {
                    ...
                    let result2 = manager.getStuff(scene: mockScene2)
        ...
        describe("remove") {
            ...
            context("2つ登録されたうち一つだけremoveする") {
                ...
                manager.set(scene: mockScene2, stuff: obj2 as AnyObject)
                let stuff1 = manager.getStuff(scene: mockScene1)
                let stuff2 = manager.getStuff(scene: mockScene2)
                ...
                it("removeされていないSceneに紐づくオブジェクトは取得できる") {
                    let result = manager.getStuff(scene: mockScene2)
                    expect(result).toNot(beNil())
