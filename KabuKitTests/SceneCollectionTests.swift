...
class SceneCollectionTests: XCTestCase {
    ...
    func test_Sceneを追加した時_その際に渡したcontextをsceneは保持している() {
        ...
        let context = "test"
        collection?.stack(scene, with: context, transition: { (stage, sce, screen) -> (() -> Void)? in
            return nil
        }, callbackOf: nil)
    ...
    func test_Sceneを追加した時_transitionが指定してあれば_それが実行される() {
        ...
        collection?.stack(scene, with: context, transition: { (stage, scene, screen) -> (() -> Void)? in
    ...
    func test_Sceneが1つ以上ある時_Sceneを追加すると前のSceneの状態がSuspendになる() {
        ...
        collection?.stack(scene1, with: context, transition: { (stage, scene, screen) -> (() -> Void)? in
            return nil
        }, callbackOf: nil)
        XCTAssertFalse(scene1.isSuspended)
        XCTAssertTrue(scene2.isSuspended)
        collection?.stack(scene2, with: context, transition: { (stage, scene, screen) -> (() -> Void)? in
            return nil
        }, callbackOf: nil)
