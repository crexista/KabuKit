...
public extension SceneSequenceBuilder where Context == Buildable, Stage == Buildable {
    ...
    public func build(onInvoke: @escaping (Guide.Stage, FirstScene) -> Void,
        ...
        return build(onInvokeWithRewind: { (stage, scene) -> (() -> Void)? in
            onInvoke(stage, scene)
            return nil
        },
                     onActive: onActive,
                     onSuspend: onSuspend,
    ...
    public func build(onInvokeWithRewind: @escaping (Guide.Stage, FirstScene) -> (() -> Void)?,
                      onActive: ((Guide.Stage, [Screen]) -> Void)? = nil,
                      onSuspend: ((Guide.Stage, [Screen]) -> Void)? = nil,
                      onLeave: ((Guide.Stage, [Screen], ReturnValue?) -> Void)? = nil) -> SceneSequence<FirstScene, Guide> {
